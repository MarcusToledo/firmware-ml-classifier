...
├── tasks.md                   # verificação retroativa por FR e cenário
└── checklists/
...
|Princípio V: `detector_version` é a constante de módulo `1.0` e não muda quando a regra do detector muda|Herdada de `src/evidence/patterns.py::_DETECTOR_VERSION` e `src/evidence/binwalk_findings.py::_DETECTOR_VERSION`|Versionar cada regra e atualizar a versão junto com qualquer mudança; correção pendente registrada no `TODO.md`|
